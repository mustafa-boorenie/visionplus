...
export class VisionAnalyzer {
   ...
   */
  static generatePrompt(analysisType: keyof typeof VisionAnalyzer.COMMON_PROMPTS): string {
    return this.COMMON_PROMPTS[analysisType];
  }

  /**
   * Analyze page for specific elements
   */
  async findElements(screenshotPath: string, elementDescription: string): Promise<string> {
    const prompt = `Find and describe all ${elementDescription} in this screenshot. For each element found, provide:
    1. Its location on the page (top, middle, bottom, left, right)
    2. Its text or label
    3. Its apparent purpose or function
    4. Any unique identifiers or attributes visible`;
  ...
  async compareScreenshots(
    ...
    1. What has changed between the first and second screenshot
    2. What remains the same
    3. Any new elements that appeared
    4. Any elements that disappeared
    ${focusAreasText}`;

    // For comparison, we'll analyze the second screenshot with context about the first
    // Note: This is a simplified approach. For true comparison, you might want to 
    // send both images in a single request if the API supports it
    const response = await this.analyzeScreenshot({
      screenshotPath: screenshot2,
      prompt: `${prompt}\n\nNote: You are looking at the second screenshot. Compare it with what would typically change from a previous state.`
    });
